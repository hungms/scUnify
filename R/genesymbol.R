...
#' @import tidyverse
#' @import biomaRt
#' @return A list containing human and mouse mart objects.
#' @export
get_biomart <- function() {
   assign("human_biomart", biomaRt::useMart("ensembl", host = 'https://dec2021.archive.ensembl.org', dataset = "hsapiens_gene_ensembl"))
   assign("mouse_biomart", biomaRt::useMart("ensembl", host = 'https://dec2021.archive.ensembl.org', dataset = "mmusculus_gene_ensembl"))}
...
convert_mouse_to_human <- function(x, unique = T){
   stopifnot(all(c("mouse_biomart", "human_biomart") %in% ls()))
